...
from m_global_set import edge_initial, u_initial, SEQLEN
...
class GN():
    def __init__(self, lr=1e-5, cuda=True):
        '''
        :param lr: 5e-4
        :param batchs: 8
        :param cuda: True
        ...
        self.Uphi = torch.load(t_dir+'uphi_%d.pth'%4).to(self.device)
        self.Ephi = torch.load(t_dir+'ephi_%d.pth'%4).to(self.device)
        self.u = torch.load(t_dir+'u_%d.pth'%4).to(self.device)
    ...
    def loadData(self):
        ...
        for camera in xrange(5, 9):
            ...
            self.train_set = DatasetFromFolder(camera, self.outName)
    ...
    def saveModel(self):
        ...
        torch.save(self.Ephi, t_dir+'ephi_%d.pth'%self.seq_index)
