...
function show_bboxes_on_video(input_frames, bboxes, tracking_id, video_fname, frame_rate, thr, output_frames, ids)
if ~exist('frame_rate')
  frame_rate = 20;
end
if ~exist('thr')
...
for i = 1:len1
  ...
  if ~isempty(bbox)
      ...
      try
        ...
        fprintf('The image does not exist!\n');
        break
      end
      if ~isempty(bbox)
        inds = find(bbox(:,end) > thr);
        %im1 = show_bbox_on_image(im1, bbox(inds, :), bws, col);
        im1 = show_bbox_on_image(ids, im1, bbox(inds, :), col);
      end

      % show how many tracks in the video
      position = [0 0];
      printStr = ['There are ', num2str(tracking_id), ' tracks in this surveillance video.'];  
