...
names = ['MOT_M_ANew_bb', 'MOT_M_ANew_bb_uupdate_tau_0.9_0.7', 'MOT_M_ANew_bb_uupdate_tau_0.5_0.5', 'MOT_M_ANew_bb_uupdate_tau']
test_seqs = [1, 3, 6, 7, 8, 12, 14]

types = ['DPM', 'FRCNN', 'SDP']


out = open('../Test%d/id_num.txt'%year, 'w')
for type in types:
    print >> out, type
    for i in xrange(len(seqs)):
        print >> out, '     ', seqs[i], lengths[i]
        for name in names:
            des_d = '../Test%d/%s/'%(year, name)
