...
class DatasetFromFolder(data.Dataset):
    ...
    def updateVelocity(self, i, j, tag=True):
        ...
        if tag:
            ...
            if i == -1:
                ...
                if num < window_size:
                    num += 1
                    self.detections[self.cur][i][3] = num+1
    ...
    def loadNext(self):
        ...
        for i in xrange(m):
            con, frame, index0, num0, id = self.detections[self.cur][i]

            k = 0
            index, num = index0, num0
            while num:
                vs[k].append(con[index])
                index, num = self.moveIndexNum(index, num)
                k += 1
        ...
        for i in xrange(self.m):
            objects, index0, num0, id = self.objects[i]
            for j in xrange(self.n):
                ...
                while num:
                    ...
                    index, num = self.moveIndexNum(index, num)
                    k += 1
                self.candidates.append([edges, self.gts[i][j], i, indexes, j])
