...
from longterm_train_dataset_fixed import DatasetFromFolder
...
class GN():
        ...
        for head in xrange(1, self.train_test):
            ...
            for epoch in xrange(1, self.nEpochs):
                ...
                for iteration in enumerate(data_loader, 1):
                    index, (edges, gt, vs_index, vs_poses, vr_index) = iteration

                    print 'Edges:', len(edges)
                    print edges
                    print 'GT: ', gt
                    print 'vs_index:', vs_index.size()
                    print vs_index
                    print 'vs_poses:', len(vs_poses)
                    print vs_poses
                    print 'vr_index:', vr_index.size()
                    print vr_index
                    # raw_input('Continue?')
                    print '---'*9
                    ...
                    for i in xrange(len(edges)):
                        e = edges[i].to(self.device)
                        tmpu = self.Uphi(self.train_set.E[i], self.train_set.V[i], self.u)
                        v1 = self.train_set.getMotion(1, vs_index, vs_poses[i])
                        v2 = self.train_set.getMotion(0, vr_index, vs_poses[i], vs_index)
                        e_.append(self.Ephi(e, v1, v2, tmpu))
                        u_.append(tmpu)

                    e_ = self.train_set.aggregate(e_)

                    # Penalize the u to let its value not too big
                    arpha = torch.mean(torch.abs(u_[0]))
    ...
    def updateUE(self):
        ...
        #     u_.append(self.Uphi(self.train_set.E[0], self.train_set.V[0], self.u))

        # self.u = u_[0].data
        self.u = self.Uphi(self.train_set.E[0], self.train_set.V[0], self.u)

        # update the edges
        # for edge in self.train_set:
        #     edges, gt, vs_index, vs_poses, vr_index = edge
        #     for i in xrange(len(edges)):
        #         e = edges.to(self.device).view(1,-1)
        #         v1 = self.train_set.getMotion(1, vs_index, vs_poses[i])
        #         v2 = self.train_set.getMotion(0, vr_index, vs_poses[i], vs_index)
        #         e_ = self.Ephi(e, v1, v2, u_[i])
        #         self.train_set.edges[vs_poses[i]][vs_index][vr_index] = e_.data.view(-1)
