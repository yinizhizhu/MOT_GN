...
class GN():
    def __init__(self, seq_index, tt, a, cuda=True):
        ...
        self.out_dir = t_dir + 'motmetrics_%s_v2_4_%.1f_%d/'%(type, a, ALPHA_TAG)
        print '		', self.out_dir
    ...
    def loadAModel(self):
        ...
        # name = 'all_v2_4'
        tail = 10
        self.AUphi = torch.load('../MOT/Results/MOT16/IoU/%s/uphi_%02d.pth'%(name, tail)).to(self.device)
        self.AEphi = torch.load('../MOT/Results/MOT16/IoU/%s/ephi_%02d.pth'%(name, tail)).to(self.device)
        self.Au = torch.load('../MOT/Results/MOT16/IoU/%s/u_%02d.pth'%(name, tail))
    ...
    def loadMModel(self):
        ...
        name = 'all_v2_4'
        tail = 10
        self.MUphi = torch.load('../MOT_Motion/Results/MOT16/IoU/%s/uphi_%d.pth'%(name, tail)).to(self.device)
        self.MEphi = torch.load('../MOT_Motion/Results/MOT16/IoU/%s/ephi_%d.pth'%(name, tail)).to(self.device)
        self.Mu = torch.load('../MOT_Motion/Results/MOT16/IoU/%s/u_%d.pth'%(name, tail))
    ...
    def evaluation(self, head, tail, gtFile, outFile):
        ...
        while a_step < tail:
            ...
            for i in xrange(len(self.a_train_set.candidates)):
                ...
                ret[a_vs_index][a_vr_index] = float(a_tmp[0]) + float(m_tmp[0])*self.alpha
