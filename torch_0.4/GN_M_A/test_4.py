...
class GN():
    ...
    def initOut(self):
        print '     Loading Data...'
        self.train_set = ADatasetFromFolder(sequence_dir, '../MOT/MOT16/train/MOT16-%02d'%self.seq_index)
    ...
    def loadModel(self):
        ...
        # name = 'all_v2_4'
        tail = 10
        self.Uphi = torch.load('../MOT/Results/MOT16/IoU/%s/uphi_%02d.pth'%(name, tail)).to(self.device)
        self.Ephi = torch.load('../MOT/Results/MOT16/IoU/%s/ephi_%02d.pth'%(name, tail)).to(self.device)
        self.u = torch.load('../MOT/Results/MOT16/IoU/%s/u_%02d.pth'%(name, tail))
