...
class GN():
    ...
    def copyLines(self, seq, head, gt_seq, tail=-1, tag=0):
        ...
        '''
        if tt_tag:
            basic_dir = 'MOT%d/test/MOT%d-%02d-%s/' % (year, year, seq, type)
        else:
            basic_dir = 'MOT%d/train/MOT%d-%02d-%s/' % (year, year, seq, type)
    ...
    def loadModel(self):
        name = 'all_det_ft_all' if app_fine_tune else 'all'
        tail = 13
        ...
        for t in types:
            ...
            for i in xrange(len(seqs)):
                ...
                if tt_tag:
                    ...
                    sequence_dir = 'MOT%d/test/'%year + seq_dir
                    print ' ', sequence_dir
                    ...
                    sequence_dir = 'MOT%d/train/'%year + seq_dir
                    print ' ', sequence_dir
