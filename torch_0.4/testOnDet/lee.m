...
%     disp(seq_index);
    ...
    tts = [];
    j = 1;
    for tt = 100:100:500 % for the inner evaluationk
        tts(j) = tt;
        j = j+1;
    end
...
%         disp(s_dir);
        ...
        for data_select=1:2
            ...
            for k=1:length(metrics)
                fprintf(fout, '\t%s', metrics{k});
            end
            fprintf(fout, '\n');

            for k=1:length(types)   %Iteration for different condition
                ind = k;
                type = sprintf('motmetrics_inner_%s', types{k});
                t_dir = fullfile(s_dir, type);
    %             disp(t_dir);

                gt_training = fullfile(t_dir, sprintf('gt_%s.txt', dataset));
                res_training = fullfile(t_dir, sprintf('res_%s.txt', dataset));
