...
class vphi(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            nn.Linear(u_num+v_num*2, 256),
            nn.LeakyReLU(inplace=True),
            nn.Linear(256, v_num),
        )

    def forward(self, v1, v2, u):
        """
        The network which updates the vertex
        :param v1: the sender
        :param v2: the receiver
        :param u: global variable
        """
        # print 'E:', v1.size(), v2.size(), u.size()
        bs = v1.size()[0]
        if bs == 1:
            tmp = u
        else:
