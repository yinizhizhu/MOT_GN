...
class GN():
    ...
    def updateNetwork(self):
        ...
        for head in xrange(1, self.train_test):
            ...
            for epoch in xrange(1, self.nEpochs):
                ...
                #   store the e1
                self.edges = [[None for i in xrange(self.train_set.m)] for j in xrange(self.train_set.n)]
                with torch.no_grad():
                    for edge in self.train_set:
                        ...
                        e1 = self.Ephi1(e, vs, vr, self.u)
                        self.edges[vr_index][vs_index] = e1.data
                ...
                for iteration in enumerate(data_loader, 1):
                    ...
                    batch_num = vr_index.size()[0]
                    if batch_num == 1:
                        e1 = self.edges[vr_index[0]][vs_index[0]]
                        vs_t = self.vertexes[vr_index[0]]
                    else:
                        e1 = torch.cat((self.edges[vr_index[0]][vs_index[0]],
                                        self.edges[vr_index[1]][vs_index[1]]), dim=0)
                        vs_t = torch.cat((self.vertexes[vr_index[0]], self.vertexes[vr_index[1]]), dim=0)
                        for i in xrange(2, batch_num):
                            e1 = torch.cat((e1, self.edges[vr_index[i]][vs_index[i]]), dim=0)
                            vs_t = torch.cat((vs_t, self.vertexes[vr_index[i]]), dim=0)

                    e1 = e1.to(self.device)
                    vr1 = self.Vphi(vs_t, vr, self.u)
                    candidates.append((e1, gt, vs, vr1))
    ...
    def updateUVE(self):
        with torch.no_grad():
            ...
            for edge in self.train_set:
                e, gt, vs_index, vr_index = edge
                e1 = self.edges[vr_index][vs_index].to(self.device)
                vs = self.train_set.getApp(1, vs_index)
                vr = self.train_set.getApp(0, vr_index)

                vs_t = self.vertexes[vr_index]
                vr1 = self.Vphi(vs_t, vr, self.u)
